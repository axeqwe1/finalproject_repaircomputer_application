        ...
        Column(Modifier.fillMaxSize()) {
            ...
            val filteredRequestList = requestList.filter { item ->
                ...
                val isInCurrentMode = when (userType) {
                    ...
                    "Technician" -> if (currentMode == "ไม่ได้กรอกข้อมูล") item.receive_repair.repair_details == null else true
                    else -> true
                }
                ...
                items(filteredRequestList) { item ->
                    OutlinedCard(
                        modifier = Modifier
                            .padding(16.dp)
                            .fillMaxWidth()
                            .clickable { navController.navigate(ScreenRoutes.detailRepair.passRridAndUserType(item.rrid.toString(),userType)) },
                        ...
                        Column(
                            ...
                            Text(
                                text = item.equipment.eq_name ?: "null",
                                fontSize = 18.sp,
                                fontWeight = FontWeight.Bold
                            )
                            Text(
